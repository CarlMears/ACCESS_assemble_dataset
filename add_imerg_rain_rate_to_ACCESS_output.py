...
def write_imerg_rain_rate_for_ACCESS(
    *,
    ...
    base_filename = get_access_output_filename_daily_folder(current_day, satellite, dataroot,'resamp_tbs')
    imerge_filename = get_access_output_filename_daily_folder(current_day, satellite, dataroot,'rain_rate_imerge_temp')
    imerge_filename_final = get_access_output_filename_daily_folder(current_day, satellite, dataroot,'rain_rate_imerge')
    ...
    with netcdf_dataset(base_filename, "r") as root_grp:
        ...
        # Create the time variables in the output file
            ...
            trg.variables[var_name].setncatts({a:var_in.getncattr(a) for a in var_in.ncattrs()})
        ...
        rr = trg.createVariable('rainfall_rate', np.float32, trg.variables['time'].dimensions,zlib=True)
