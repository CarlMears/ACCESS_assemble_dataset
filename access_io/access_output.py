...
def append_var_to_daily_tb_netcdf(
    *,
    ...
    filename = get_access_output_filename(
        year=year, month=month, day=day, satellite=satellite, dataroot=dataroot
    )
    ...
    try:
        ...
        if overwrite:
            ...
            raise RuntimeError(
                f"Error -- Variable {var_name} already exists and overwrite is False, so raising error"
            )
...
def append_lf_daily_tb_netcdf(
    *, year, month, day, satellite, land_fraction, dataroot="L:/access/"
):
...
def write_daily_tb_netcdf(
    *,
    ...
    root_grp.keywords_vocabulary = (
        "NASA Global Change Master Directory (GCMD) Earth Science Keywords, Version 6.0"
    )
