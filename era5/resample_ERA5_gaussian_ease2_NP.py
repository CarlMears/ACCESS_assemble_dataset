...
if __name__ == "__main__":
    ...
    for iy in np.arange(0, 720):
        for ix in np.arange(0, 720):
            latitude0 = ease2_grid_25km_north.latitude[iy, ix]
            longitude0 = ease2_grid_25km_north.longitude[iy, ix]
            if longitude0 < 0.0:
            ...
            # The data is in lat/lon coordinates.  To
            # integrate over the footprint, we need it to be
            # in x,y (kilometer) coordinates.  Fortunately,
            # the pyproj projection package does just this.
            # The warnings from this package are suppressed.
            # Something about the way things are called
            # here are deprecated (including internally
            # to the package).  This needs to be cleaned up at
            # some point, but for now it works.
            #
            # construct a local projection, centered at the center
            # of the target gaussian.
        ...
        nc_file = (
            f"L:/access/era5/resample_weights/"
            f"resamp_wts_ease2_25km_NP_{int(footprint_diameter_km)}km.nc"
        )
