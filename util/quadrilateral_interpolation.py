...
class QuadrilateralTranform:
    '''Finds a bilinear transformation from an arbitrary quadrilateral to a unit square'''

    '''wish list: test and fix quadrilaterals presented in the wrong order.'''
    '''wish list: test to make sure points inside a quadrilateral never give an error
    ...
    def __init__(self,
        ...
        self.bb_part = self.a[3]*self.b[0] - self.a[0]*self.b[3] + self.a[1]*self.b[2] - self.a[2]*self.b[1]
    ...
    def __call__(self,x,y):
        ...
        if ((l < 0.0) or (l > 1.0) or (m < 0.0) or (m > 1.0)):
            raise ValueError('x,y not inside quadrilateral')
...
class InterpParallelogram:
    '''Interpolation inside an arbitratry parallelogram using a remapped bilinear interpolator'''

    def __init__(self,
                 x,   # array/list of 4 x locations
                 y,   # array/list of 4 y locations
                 z):   # array/list of 4 z locations)
        ...
        # the interpolation is done on the unit square, where the interpolated value is given by
        ...
        #need to find the transform that transforms the parallelogram onto the unit square
        #
        #  | a b | |vec01x|    | 1 |
        #  |     |x|      | =  |   |
        #  | c d | |vec01y|    | 0 |
    ...
    def check_if_parallelogram(self,rel_threshold = 0.005):
        ...
        threshold = rel_threshold*0.5*(np.sqrt(self.len_01_sqr)+np.sqrt(self.len_03_sqr))
    ...
    def __call__(self,x0,y0,check_for_inside=True):
        ...
        #This transforms the locations dx,dy (relative to x[0],y[0]) onto the unit square
...
class InterpQuadrilateralFit:
    ...
    def __call__(self,x,y):
        ...
        return self.parameters[0]*np.square(x) + \
    ...
    def check_inside(self,x,y):
        '''checks to see if x,y is inside the quadrilateral defined for the interpolator'''

        # Not implemented yet
        # I know how to do it for a convex quadrilateral, but not for one that is not convex
        # probably it is not very good to do this for a non convex quadrilateral anyway....
        pass
...
if __name__ == '__main__':
    ...
    for i in range(4):
        ...
        if np.abs(qi(x[i],y[i]) - z[i]) > a_small_number:
            raise Exception('Fail - error in interploated value at a corner of the quadrilateral')
    inside_test_point = [2.0,1.0]
    print(f'Interpolated Value {inside_test_point[0]:.2f},{inside_test_point[1]:.2f} = {qi(inside_test_point[0],inside_test_point[1])}')
    ...
    for i in range(4):
        ...
        if np.abs(pi(x[i],y[i]) - z[i]) > a_small_number:
            raise Exception('Fail - error in interploated value at a corner of the quadrilateral')
    inside_test_point = [2.0,1.0]
    print(f'Interpolated Value {inside_test_point[0]:.2f},{inside_test_point[1]:.2f} = {pi(inside_test_point[0],inside_test_point[1])}')
    ...
    for i in range(4):
        print(f'Interpolated Value ax {x[i]},{y[i]} = {sci(x[i],y[i])} should be {z[i]}')
        if np.abs(sci(x[i],y[i]) - z[i]) > a_small_number:
            raise Exception('Fail - error in interploated value at a corner of the quadrilateral')
        
    inside_test_point = [2.0,1.0]
    print(f'Interpolated Value {inside_test_point[0]:.2f},{inside_test_point[1]:.2f} = {sci(inside_test_point[0],inside_test_point[1])}')
    inside_test_point = [2.5,3.0]
    print(f'Interpolated Value {inside_test_point[0]:.2f},{inside_test_point[1]:.2f} = {sci(inside_test_point[0],inside_test_point[1])}')
    inside_test_point = [2.5,1.0]
    print(f'Interpolated Value {inside_test_point[0]:.2f},{inside_test_point[1]:.2f} = {sci(inside_test_point[0],inside_test_point[1])}')
    ...
    try:
        print(f'Interpolated Value {outside_test_point[0]:.2f},{outside_test_point[1]:.2f} = {pi(outside_test_point[0],inside_test_point[1])}')
