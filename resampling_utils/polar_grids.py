...
def polar_stereo_interp_SP(polar_map,lats,lons):
    ...
    interp_ok = np.all([((y_lower+158) >= 0),((y_upper+158) <= 331),((x_lower+158)>= 0),((x_upper+158)<=315)],axis=(0))
    ...
    total[ok_finite] = total[ok_finite]+upper_right[ok_finite]*wt_upper_right[ok_finite]
    total_wt[ok_finite] = total_wt[ok_finite] + wt_upper_right[ok_finite]
    
    ok_finite = np.isfinite(upper_left)
    total[ok_finite] = total[ok_finite]+upper_left[ok_finite]*wt_upper_left[ok_finite]
    total_wt[ok_finite] = total_wt[ok_finite] + wt_upper_left[ok_finite]
    
    ok_finite = np.isfinite(lower_right)
    total[ok_finite] = total[ok_finite]+lower_right[ok_finite]*wt_lower_right[ok_finite]
    total_wt[ok_finite] = total_wt[ok_finite] + wt_lower_right[ok_finite]
    
    ok_finite = np.isfinite(lower_left)
    total[ok_finite] = total[ok_finite]+lower_left[ok_finite]*wt_lower_left[ok_finite]
...
def polar_stereo_interp(polar_map,lats,lons):
    ...
    interp_ok = np.all([((y_lower+234) >= 0),((y_upper+234) <= 447),((x_lower+154)>= 0),((x_upper+154)<=303)],axis=(0))
    interp_value = np.zeros(lats.shape)
    interp_value2 = np.zeros(lats.shape)
...
def polarstereo_fwd(lats,lons,r_e = 6378.2730,e = 0.081816153,std_parallel = 70.0,lon_y = -45.0):
#   
...
#
    ...
    #   pm=-1 !plus or minus, north lat. or south
        ...
        t  =np.tan(np.pi/4.0 - phi/2.0)/  np.power(((1-e*np.sin(phi))/  (1+e*np.sin(phi))),(e/2))    # eq 15-9, pg 161
        t_c=np.tan(np.pi/4   - phi_c/2.0)/np.power(((1-e*np.sin(phi_c))/(1+e*np.sin(phi_c))),(e/2))  # eq 15-9, pg 161
        
        m  = np.cos(phi)/np.sqrt(1-e*e*np.square(np.sin(phi)))       # eq 14-15, pg 160
        m_c= np.cos(phi_c)/np.sqrt(1-e*e*np.square(np.sin(phi_c)))   # eq 14-15, pg 160
...
def polarstereo_fwd_SP(lats,lons,r_e = 6378.2730,e = 0.081816153,std_parallel = 70.0,lon_y = 180.0):
#   
#   The National Snow and Ice Data Center (NSIDC) and Scientific Committee
#   on Antarctic Research (SCAR) use a version of the polar stereographic
#   projection.
...
# A copy of the paper is in X:\DOCUMENT\Journal_Papers\Snyder_1987_projections_USGS.pdf
    ...
    with np.errstate(invalid='ignore'):
        ...
        sl = std_parallel*np.pi/180.
        
        rho=np.sqrt(x*x + y*y)
    
        m_c = np.cos(sl)/np.sqrt(1.0-e2*(np.power(np.sin(sl),2)))                # eq 14-15, pg 160
        t_c=np.tan((np.pi/4.0)-(sl/(2.0)))/np.power(((1.0-e*np.sin(sl))/(1.0+e*np.sin(sl))),(e/2.0))  # eq 15-9, pg 161
        if (np.abs(std_parallel-90.) < 1.E-5):
            t=rho*np.sqrt((np.power((1.0+e),(1.0+e))*np.power((1.0-e),(1.0-e))))/(2.*r_e)   #eq 21-39, pg 162
        else:
            t=rho*t_c/(r_e*m_c)         #eq 21-40 pg 162
            
        chi = (np.pi/2.0)-2.0*np.arctan(t)   #eq 7-13, pg 162
        alat = chi +  ((e2/2.0)+(5.0*e4/24.0)+(e6/12.0)+(13*e8/360))*np.sin(2.0*chi)   # eq 3-5, pg 162  
        alat = alat + ((7.0*e4/48.0)+(29.0*e6/240.0)+(811.0*e8/11520))*np.sin(4.0*chi) # this is an approximation to the true inverse
        alat = alat + ((7.0*e6/120.0)+(81.0*e8/1120))*np.sin(6.0*chi)                  # it is accurate to more or less machine precision for locations near the pole.
...
#      real(8) q_theta, sin_lat, e_sin_lat
      ...
      q_theta = (1 - ecc*ecc) * (sin_lat / (1 - e_sin_lat*e_sin_lat) - 1 / (2 * ecc) * np.log((1 - e_sin_lat)/(1 + e_sin_lat)))
...
class NSIDC_ease2_grids():
    ...
    def __init__(self, pole = 'north', resolution = '25km'):
        ...
        lats,lons,crs = _load_NSIDC_ease2_grids(
                        pole = pole,
...
def _load_NSIDC_ease2_grids(pole = 'north',
                        resolution = '25km'):
    ...
    if pole == 'north':
        ...
        raise ValueError(f'pole = {pole} not valid in polar_grids.load_NSIDC_ease2_grids')
...
#     from WGS84
      ...
      if south_pole:
          x = 2.0 * r * np.sin((lon-lambda0)*np.pi/180.0) *  np.sin(np.pi/4 - 0.5*lat*np.pi/180.0)  #eq 24-8, pg 186
          y = 2.0 * r * np.cos((lon-lambda0)*np.pi/180.0) *  np.sin(np.pi/4 - 0.5*lat*np.pi/180.0)  #eq 24-9, pg 186
      else:
          x = 2.0 * r * np.sin((lon-lambda0)*np.pi/180.0) *  np.sin(np.pi/4 - 0.5*lat*np.pi/180.0)  #eq 24-3, pg 186
          y = -2.0 * r * np.cos((lon-lambda0)*np.pi/180.0) *  np.sin(np.pi/4 - 0.5*lat*np.pi/180.0) #eq 24-4, pg 186
...
def modis_tile_inv(vertical_tile,horizontal_tile,line,sample,r = 6371.22800,lambda0 = 0.0,south_pole = False):
...
if __name__ == '__main__':
    ...
    #
        ...
        test_lons = np.array([168.35,135.00,102.34,45.00,350.03,315.00,279.26,225.0])
        ...
        x, y = polarstereo_fwd(test_lats, test_lons, r_e=6378.2730, e=0.081816153, std_parallel=70.0)
        print('North Pole')
        for i, lat in enumerate(test_lats):
            print(("{:10.3f}".format(test_lats[i]),
        ...
        test_lats = np.array([-39.23,-51.32,-39.23,-54.66,-41.45,-54.66,-41.45,-54.66])
        ...
        x,y = polarstereo_fwd_SP(test_lats,test_lons,r_e = 6378.2730,e = 0.081816153,std_parallel = 70.0,lon_y = 180.0)
        print('South Pole')
        for i, lat in enumerate(test_lats):
            print(("{:10.3f}".format(test_lats[i]),
                  "{:10.3f}".format(test_lons[i]),
